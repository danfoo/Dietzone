        ...
        if ($http_code === 201 && isset($result['status']) && $result['status'] === 'COMPLETED') {
            ...
            if ($invoice->status != 2) {
                ...
                if ($payment_recorded) {
                    ...
                    // Get patient ID from invoice client
                    $patient = $this->db->get_where(db_prefix() . 'dietic_patients', ['client_id' => $client_id])->row();

                    if ($patient) {
                        log_activity('PAYPAL CALLBACK SUCCESS - Found patient ID: ' . $patient->id);

                        // Get the service/item that was purchased
                        $invoice_items = $this->db->get_where(db_prefix() . 'itemable', [
                            'rel_id' => $invoice_id,
                            'rel_type' => 'invoice'
                        ])->result();

                        if (!empty($invoice_items)) {
                            $item = $invoice_items[0]; // Get first item

                            // Create a service object for subscription creation
                            $service = (object)[
                                'id' => $item->description, // Use description as identifier
                            ...
                            log_activity('PAYPAL CALLBACK SUCCESS - Creating subscription for service: ' . $service->description);

                            // Create subscription with auto-assigned dietitian
                            $subscription_id = dietetic_create_subscription_from_payment(
                                $patient->id,
                                $invoice_id,
                                $service
                            );
                            ...
                            if ($subscription_id) {
                                log_activity('PAYPAL CALLBACK SUCCESS - Subscription created: ' . $subscription_id);

                                // Create revenue share record
                                log_activity('PAYPAL CALLBACK SUCCESS - Creating revenue share record');
                                $revenue_share_id = dietetic_create_revenue_share(
                                    $subscription_id,
                                    $invoice_id,
                                    $invoice->total,
                                    $payment_id
                                ...
                                if ($revenue_share_id) {
                                    log_activity('PAYPAL CALLBACK SUCCESS - Revenue share created: ' . $revenue_share_id);
                                } else {
                                    log_activity('PAYPAL CALLBACK SUCCESS - Warning: Failed to create revenue share');
                                }

                                // Create automatic program for the patient
                                log_activity('PAYPAL CALLBACK SUCCESS - Creating automatic program for patient');
                                try {
                                    $this->load->model('dietetic/dietetic_programs_model');
                                    $this->load->model('dietetic/dietetic_subscriptions_model');
                                    ...
                                    if ($subscription && $subscription->dietitian) {
                                        $program_data = [
                                            'patient_id' => $patient->id,
                                            'dietitian_id' => $subscription->dietitian_id,
                                            'name' => 'Programme ' . $service->description,
                                            'description' => 'Programme automatique créé suite à votre souscription au service: ' . $service->description,
                                            'start_date' => date('Y-m-d'),
                                            'end_date' => $subscription->end_date ?? date('Y-m-d', strtotime('+1 month')),
                                            'status' => 'active',
                                            'created_by' => $subscription->dietitian_id
                                        ];

                                        $program_id = $this->dietetic_programs_model->add($program_data);

                                        if ($program_id) {
                                            log_activity('PAYPAL CALLBACK SUCCESS - Program created successfully: ' . $program_id);
                                        } else {
                                            log_activity('PAYPAL CALLBACK SUCCESS - Warning: Failed to create program');
                                        }
                                    }
                                } catch (Exception $e) {
                                    log_activity('PAYPAL CALLBACK SUCCESS - Program creation exception: ' . $e->getMessage());
                                }

                                // Send confirmation emails
                                log_activity('PAYPAL CALLBACK SUCCESS - Sending confirmation emails');
                                try {
                                    $this->load->model('dietetic/dietetic_subscriptions_model');
                                    $subscription = $this->dietetic_subscriptions_model->get($subscription_id);

                                    if ($subscription && $subscription->patient && $subscription->dietitian) {
                                        ...
                                        if ($client) {
                                            ...
                                            $patient_email_data = [
                                                ...
                                                'dietitian_name' => $subscription->dietitian->firstname . ' ' . $subscription->dietitian->lastname,
                                                'dietitian_email' => $subscription->dietitian->email,
                                                'amount' => $invoice->total,
                                            ...
                                            $this->email->subject('Souscription confirmée - ' . $service->description);

                                            $message = "Bonjour " . $client->company . ",\n\n";
                                            $message .= "Votre paiement de " . $invoice->total . " XOF a été confirmé avec succès.\n\n";
                                            $message .= "Détails de votre souscription:\n";
                                            $message .= "- Service: " . $service->description . "\n";
                                            $message .= "- Diététicien assigné: " . $subscription->dietitian->firstname . ' ' . $subscription->dietitian->lastname . "\n";
                                            $message .= "- Email diététicien: " . $subscription->dietitian->email . "\n";
                                            $message .= "- Date de début: " . date('d/m/Y', strtotime($subscription->start_date)) . "\n";
                                            $message .= "- Date de fin: " . date('d/m/Y', strtotime($subscription->end_date)) . "\n\n";
                                            $message .= "Votre diététicien vous contactera prochainement pour commencer votre suivi.\n\n";
                                            ...
                                            if ($this->email->send()) {
                                                log_activity('PAYPAL CALLBACK SUCCESS - Patient confirmation email sent to: ' . $client->email);
                                            } else {
                                                log_activity('PAYPAL CALLBACK SUCCESS - Failed to send patient email: ' . $this->email->print_debugger());
                                            }

                                            // Email to dietitian
                                            $this->email->clear();
                                            $this->email->from(get_option('smtp_email'), get_option('companyname'));
                                            $this->email->to($subscription->dietitian->email);
                                            $this->email->subject('Nouveau patient assigné - ' . $client->company);

                                            $message = "Bonjour " . $subscription->dietitian->firstname . ",\n\n";
                                            $message .= "Un nouveau patient vous a été assigné suite à une souscription.\n\n";
                                            $message .= "Détails du patient:\n";
                                            $message .= "- Nom: " . $client->company . "\n";
                                            $message .= "- Email: " . $client->email . "\n";
                                            $message .= "- Téléphone: " . ($client->phonenumber ?? 'N/A') . "\n";
                                            $message .= "- Service souscrit: " . $service->description . "\n";
                                            $message .= "- Montant: " . $invoice->total . " XOF\n";
                                            $message .= "- Période: du " . date('d/m/Y', strtotime($subscription->start_date)) . " au " . date('d/m/Y', strtotime($subscription->end_date)) . "\n\n";
                                            $message .= "Veuillez contacter votre patient pour commencer le suivi.\n\n";
                                            $message .= "Accéder au dossier patient: " . admin_url('dietetic/patients/view/' . $patient->id) . "\n\n";
                                            $message .= "Cordialement,\n";
                                            $message .= get_option('companyname');
                                            ...
                                            if ($this->email->send()) {
                                                log_activity('PAYPAL CALLBACK SUCCESS - Dietitian notification email sent to: ' . $subscription->dietitian->email);
                                            } else {
                                                log_activity('PAYPAL CALLBACK SUCCESS - Failed to send dietitian email: ' . $this->email->print_debugger());
                                            }
                                        }
                                    }
                                } catch (Exception $e) {
                                    log_activity('PAYPAL CALLBACK SUCCESS - Email sending exception: ' . $e->getMessage());
                                }

                                log_activity('PAYPAL CALLBACK SUCCESS - All post-payment processing completed');
                            } else {
                                log_activity('PAYPAL CALLBACK SUCCESS - Warning: Failed to create subscription');
                            }
                        } else {
                            log_activity('PAYPAL CALLBACK SUCCESS - Warning: No invoice items found');
                        }
                    } else {
                        log_activity('PAYPAL CALLBACK SUCCESS - Warning: Patient not found for client ID: ' . $client_id);
                    }
                } else {
