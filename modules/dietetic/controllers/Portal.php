...
";
    ...
    /**
     * Get Firebase configuration (AJAX)
     */
    public function get_firebase_config()
    {
        header('Content-Type: application/json');
        ...
        // Get Firebase config
        $config = [
            'apiKey' => $this->dietetic_notifications_model->get_setting('firebase_api_key'),
            'authDomain' => $this->dietetic_notifications_model->get_setting('firebase_auth_domain'),
            'projectId' => $this->dietetic_notifications_model->get_setting('firebase_project_id'),
            'storageBucket' => $this->dietetic_notifications_model->get_setting('firebase_storage_bucket'),
            'messagingSenderId' => $this->dietetic_notifications_model->get_setting('firebase_messaging_sender_id'),
            'appId' => $this->dietetic_notifications_model->get_setting('firebase_app_id')
        ];

        // Check if all required fields are present
        if (empty($config['apiKey']) || empty($config['projectId']) || empty($config['messagingSenderId']) || empty($config['appId'])) {
            echo json_encode([
                'success' => false,
    ...
    public function save_fcm_token()
    {
        ...
        // Get current patient
        $patient_id = get_patient_user_id();
        ...
        try {
            ...
            $existing = $this->db->get(db_prefix() . 'dietic_fcm_tokens')->row();

            if ($existing) {
                // Update existing token
                $this->db->where('id', $existing->id);
                $this->db->update(db_prefix() . 'dietic_fcm_tokens', [
                    'is_active' => 1,
                ...
                $this->db->insert(db_prefix() . 'dietic_fcm_tokens', $token_data);
            ...
            log_activity('save_fcm_token ERROR: ' . $e->getMessage());
            echo json_encode([
                'success' => false,
